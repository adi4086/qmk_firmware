...
static inline matrix_row_t read_cols(void) {
    ...
    uint16_t portD_pin_state = palReadPort(PAL_PORT(D0));
    return ((((portA_pin_state & 0b11110000) ^ 0b11110000) >> 4) | (((portB_pin_state & 0b11) ^ 0b11) << 4) | (((portB_pin_state & 0b1111110000000000) ^ 0b1111110000000000) >> 4) | (((portA_pin_state & 0b11100000000) ^ 0b11100000000) << 4) | ((portA_pin_state & 0b1000000000000000) ^ 0b1000000000000000) | (((portB_pin_state & 0b1000) ^ 0b1000) << 13) | (((portC_pin_state & 0b1110000000000) ^ 0b1110000000000) << 7) | (((portD_pin_state & 0b100) ^ 0b100) << 18));
}
...
uint8_t matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    for (uint8_t current_row = 0; current_row < MATRIX_ROWS; current_row++) {
        ...
        while (stable_threshold > 0) { // Wait for all Col signals to go HIGH
            stable_threshold = ((((palReadPort(PAL_PORT(A0)) & colA_bits) ^ colA_bits) | ((palReadPort(PAL_PORT(B0)) & colB_bits) ^ colB_bits) | ((palReadPort(PAL_PORT(C0)) & colC_bits) ^ colC_bits) | ((palReadPort(PAL_PORT(D0)) & colD_bits) ^ colD_bits)) == 0) ? (stable_threshold - 1) : MATRIX_DEBOUNCE;
        }
